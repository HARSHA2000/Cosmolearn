...
const FROM = () => process.env.RESEND_FROM_EMAIL || "noreply@cosmoverge.in";
const TRAINER_EMAIL = () =>
...
export async function sendTrainerNotification(lead: Lead) {
  return getResend().emails.send({
    from: FROM(),
    to: TRAINER_EMAIL(),
    subject: `New Proposal Request — ${lead.college_name} (${lead.program_interest})`,
    html: `
...
export async function sendAcknowledgementEmail(lead: Lead) {
  return getResend().emails.send({
    from: FROM(),
    to: lead.email,
    subject: `Got your request, ${lead.full_name.split(" ")[0]}! — Harshith AI/ML Trainer`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
        <div style="background:#0f172a;padding:24px 32px;border-radius:12px 12px 0 0;">
          <h1 style="color:#f59e0b;font-size:22px;margin:0;">Thanks for reaching out!</h1>
          <p style="color:#94a3b8;margin:8px 0 0;">Harshith | AI/ML Trainer, Bengaluru</p>
        </div>
        <div style="background:#fff;padding:24px 32px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px;">
          ...
          <p style="color:#0f172a;margin-top:24px;">
            Talk soon,<br/>
            <strong>Harshith</strong><br/>
            <span style="color:#64748b;font-size:13px;">AI/ML Trainer | Cosmoverge, Bengaluru</span>
          </p>
        </div>
      </div>
    `,
